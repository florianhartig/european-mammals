...
The compendium behind the *Atlas of European Mammals* decided at some point, that they would divide Europe into rectangular units. Each of these units would contain information about the occurence of a species in this unit. This grid was for comparison geared to the before used grid system of the [*Atlas Florae Europaeae*](http://www.luomus.fi/en/new-grid-system-atlas-florae-europaeae).
Consequently all of these grid units were drawn into the image, or with other words, coded into the textfile which would be turned into the image later and provided on their website. Fortunately the description of each grid unit also contains a systematic name, which we can use for our purpose. Naturally the information of ocurrence of a certain species is also given per grid in the text and that makes it possible to extract and combine these information and start deriving additional information from it.
